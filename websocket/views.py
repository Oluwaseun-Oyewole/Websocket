...
def send_message(request):
    ...
    message = dictionary_body['body']['content']
    ChatMessage.objects.create(username=username, timestamp=timestamp, message=message)
    connections = Connection.objects.get(id=1)
    messages = {
        "username":username,
        "timestamp":timestamp,
    }
